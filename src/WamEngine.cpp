...
void WamEngine::initWindow()
{
	...
	window = SDL_CreateWindow(
		"Am SDL2 Vulkan window",					// window title
	...
	if (window == NULL)
	{
		// In the cas that window could not be mad...
...
void WamEngine::CreateInstance()
{
	...
	appInfo.pApplicationName = "WamEngineAPP";
	appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
	appInfo.pEngineName = "WamEngine";
...
std::vector<const char*> WamEngine::GetRequiredExtensions()
{
	...
	size_t additional_extensin_count = extensions.size();
	extensions.resize(additional_extensin_count + extensionCount);

	if (!SDL_Vulkan_GetInstanceExtensions(window, &extensionCount, extensions.data() + additional_extensin_count))
	{
		throw std::runtime_error("Could not SDL Vulkan Extension correctly");
	}
...
{
	VkPipelineInputAssemblyStateCreateInfo inputAssembly = PipelineInputAssemblyStateInitializer(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, VK_FALSE);
	VkPipelineRasterizationStateCreateInfo rasterizer = PipelineRasterizationStateInitializer(VK_POLYGON_MODE_FILL, VK_CULL_MODE_BACK_BIT, VK_FRONT_FACE_COUNTER_CLOCKWISE);
	VkPipelineColorBlendAttachmentState colorBlendAttachment = PipelineColorBlendAttachmentInitializer(
			VK_COLOR_COMPONENT_R_BIT |
			VK_COLOR_COMPONENT_G_BIT |
			VK_COLOR_COMPONENT_B_BIT |
			VK_COLOR_COMPONENT_A_BIT,
			VK_FALSE);
	VkPipelineColorBlendStateCreateInfo colorBlending = PipelineColorBlendStateInitializer(1, &colorBlendAttachment);
	// viewport
	VkPipelineDepthStencilStateCreateInfo depthStencil = PipelineDepthStencilStateInitializer(VK_TRUE, VK_TRUE, VK_COMPARE_OP_LESS);
	VkPipelineMultisampleStateCreateInfo multisampling = PipelineMultisampleStateInitializer(msaaSamples);
	std::array<VkPipelineShaderStageCreateInfo, 2> shaderStages;

	auto bindingDescriptions = Vertex::getBindingDescriptions();
	auto attributeDescriptions = Vertex::getAttributeDescriptions();

	VkPipelineVertexInputStateCreateInfo vertexInputInfo = PipelineVertexInputStateInitializer();
	...
	std::vector<VkDynamicState> dynamicStateEnable = { VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR };
	VkPipelineDynamicStateCreateInfo dynamicState = PipelineDynamicStateInitializer(dynamicStateEnable);

	VkPipelineViewportStateCreateInfo viewportState = PipelineViewportStateInitializer(1, 1);
	...
	std::array<VkVertexInputAttributeDescription, 2> skyBoxAttributeDescriptions = {};
	...
	skyBoxAttributeDescriptions[0].offset = offsetof(Vertex, pos);

	skyBoxAttributeDescriptions[0].binding = 0;
	skyBoxAttributeDescriptions[0].location = 1;
	skyBoxAttributeDescriptions[0].format = VK_FORMAT_R32G32_SFLOAT;
	skyBoxAttributeDescriptions[0].offset = offsetof(Vertex, texCoord);

	vertexInputInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(skyBoxAttributeDescriptions.size());
	vertexInputInfo.pVertexAttributeDescriptions = skyBoxAttributeDescriptions.data();
...
void WamEngine::CreateDescriptorSetLayout()
{
	...
	samplerLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; // should use VK_DESCRIPTOR_TYPE_STORAGE_BUFFER
...
void WamEngine::CreateTextureImage()
{
	for(int i = 0; i < models.size(); i++)
	{
		...
		stbi_uc* pixels = stbi_load(texturesPaths[i].c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
		...
		CreateImage(
			...
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
			models[i].textureImage,
			models[i].textureImageMemory);
...
void WamEngine::CreateTextureSampler()
{
	for(int i = 0; i < models.size(); i++)
	{
		...
		samplerInfo.minLod = static_cast<float>(0); // Optional
		samplerInfo.maxLod = static_cast<float>(models[i].mipLevels);
		samplerInfo.mipLodBias = 0; // Optional

		if (vkCreateSampler(device, &samplerInfo, nullptr, &models[i].textureSampler) != VK_SUCCESS)
		{
...
void WamEngine::CreateCommandBuffers()
{
	...
	for (size_t i = 0; i < commandBuffers.size(); i++) {
		...
		vkCmdBindPipeline(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelines.lightPillar);

		VkBuffer vertexBuffers[] = { models[0].vertexBuffer };
		VkDeviceSize offsets[] = { 0 };
		vkCmdBindVertexBuffers(commandBuffers[i], 0, 1, vertexBuffers, offsets);

		vkCmdBindIndexBuffer(commandBuffers[i], models[0].indexBuffer, 0, VK_INDEX_TYPE_UINT32);
...
void WamEngine::CreateUniformBuffers()
{
	...
	for (size_t i = 0; i < swapChainImages.size(); i++) {
		Buffer::CreateBuffer(device, physicalDevice, bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, uniformBuffers[i], uniformBufferMemory[i]);
	}
}
...
inline VkPipelineInputAssemblyStateCreateInfo WamEngine::PipelineInputAssemblyStateInitializer(VkPrimitiveTopology topology, VkBool32 primitiveRestartEnable)
{
...
inline VkPipelineRasterizationStateCreateInfo WamEngine::PipelineRasterizationStateInitializer(VkPolygonMode polygonMode, VkCullModeFlags cullMode, VkFrontFace frontFace)
{
...
inline VkPipelineColorBlendAttachmentState WamEngine::PipelineColorBlendAttachmentInitializer(VkColorComponentFlags colorWriteMask, VkBool32 blendEnable)
{
...
inline VkPipelineColorBlendStateCreateInfo WamEngine::PipelineColorBlendStateInitializer(uint32_t attachmentCount, const VkPipelineColorBlendAttachmentState * pAttachement)
{
	VkPipelineColorBlendStateCreateInfo colorBlendState = {};

	colorBlendState.sType = VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO;
	colorBlendState.attachmentCount = attachmentCount;
	colorBlendState.pAttachments = pAttachement;

	return colorBlendState;
}

inline VkPipelineDepthStencilStateCreateInfo WamEngine::PipelineDepthStencilStateInitializer(VkBool32 depthTestEnable, VkBool32 depthWriteEnable, VkCompareOp depthCompareOp)
{
...
inline VkPipelineViewportStateCreateInfo WamEngine::PipelineViewportStateInitializer(uint32_t viewportCount, uint32_t scissorCount)
{
...
inline VkPipelineMultisampleStateCreateInfo WamEngine::PipelineMultisampleStateInitializer(VkSampleCountFlagBits rasterizationSamples)
{
	VkPipelineMultisampleStateCreateInfo multisapleState = {};

	multisapleState.sType = VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO;
	multisapleState.rasterizationSamples = rasterizationSamples;

	return multisapleState;
}

inline VkPipelineDynamicStateCreateInfo WamEngine::PipelineDynamicStateInitializer(const std::vector<VkDynamicState>& pDynamicStates)
{
...
inline VkPipelineVertexInputStateCreateInfo WamEngine::PipelineVertexInputStateInitializer()
{
...
VkPipelineShaderStageCreateInfo WamEngine::LoadShader(std::string shaderPath, VkShaderStageFlagBits shaderTypeFlag)
{
	VkPipelineShaderStageCreateInfo shaderStage = {};

	shaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
	shaderStage.stage = shaderTypeFlag;
	VkShaderModule shadermodule = CreateShaderModule(ReadFile(shaderPath));
