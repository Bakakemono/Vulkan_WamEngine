...
void WamEngine::CleanUpSwapChain()
{
	...
	for (int j = 0; j < models.size(); j++)
	{
		for (size_t i = 0; i < swapChainImages.size(); i++) {
			...
			vkFreeMemory(device, uniformBufferMemory[j][i], nullptr);
		}
	}
...
UpdateUniformBuffer(uint32_t currentImage)
{
	...
	for(int i = 0; i < models.size(); i++)
	{
		...
		vkUnmapMemory(device, uniformBufferMemory[i][currentImage]);
	}
}
...
void WamEngine::UpdateDescriptorSets(int modelIndex)
{
	for (size_t i = 0; i < swapChainImages.size(); i++) {
		...
		imageInfo.imageView = models[modelIndex].textureImageView;
		imageInfo.sampler = models[modelIndex].textureSampler;
...
void WamEngine::CreateUniformBuffers()
{
	for(int i = 0; i < models.size(); i++)
	{
		...
		uniformBufferMemory.resize(i + 1);

		uniformBuffers[i].resize(swapChainImages.size());
		uniformBufferMemory[i].resize(swapChainImages.size());

		for (size_t j = 0; j < swapChainImages.size(); j++) {
			Buffer::CreateBuffer(device, physicalDevice, bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, uniformBuffers[i][j], uniformBufferMemory[i][j]);
		}
	}
}
